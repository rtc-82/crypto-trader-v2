...
class MeanReversionStrategy:
    ...
    def on_candle(self, close: float, high: float, low: float, timestamp=None) -> Optional[Signal]:
        self._closes.append(float(close))
        atr = self._update_atr(close, high, low)

        if len(self._closes) < self.cfg.bb_period or atr is None:
            return None
        if atr <= 0 or len(self._atr_hist) < self.cfg.min_atr_history:
            return None
        ...
        # If price is far ABOVE mean => SHORT
        if z >= self.cfg.z_entry:
            return Signal(direction="SHORT", atr=float(atr))

        # If price is far BELOW mean => LONG
        if z <= -self.cfg.z_entry:
            return Signal(direction="LONG", atr=float(atr))
